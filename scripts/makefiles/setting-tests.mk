...
test-setting-list:
	...
	curl -X GET "http://localhost:8081/api/v1/$$URL_CODE/settings" \
...
test-setting-get:
	...
	TOKEN=$$(echo "$$LOGIN_RESPONSE" | grep -o '"token":"[^"]*' | cut -d'"' -f4); \
	URL_CODE=$$(echo "$$LOGIN_RESPONSE" | grep -o '"current_tenant"[^}]*"url_code":"[^"]*' | grep -o '"url_code":"[^"]*' | cut -d'"' -f4); \
	curl -X GET "http://localhost:8081/api/v1/$$URL_CODE/settings/interface" \
		-H "Authorization: Bearer $$TOKEN"
	@echo ""
...
test-setting-update:
	...
	TOKEN=$$(echo "$$LOGIN_RESPONSE" | grep -o '"token":"[^"]*' | cut -d'"' -f4); \
	URL_CODE=$$(echo "$$LOGIN_RESPONSE" | grep -o '"current_tenant"[^}]*"url_code":"[^"]*' | grep -o '"url_code":"[^"]*' | cut -d'"' -f4); \
	curl -X PUT "http://localhost:8081/api/v1/$$URL_CODE/settings/interface" \
		-H "Content-Type: application/json" \
		-H "Authorization: Bearer $$TOKEN" \
...
test-settings-all:
	...
	curl -s -X PUT "http://localhost:8081/api/v1/$$URL_CODE/settings/interface" \
		-H "Content-Type: application/json" \
		-H "Authorization: Bearer $$TOKEN" \
